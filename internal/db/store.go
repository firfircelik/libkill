...
func (s *Store) UpsertThreats(entries []ThreatEntry) (int, error) {
	inserted := 0
	query := `INSERT OR IGNORE INTO threats (package, version, ecosystem, feed, severity, reason, detected)
	           VALUES (?, ?, ?, ?, ?, ?, ?)`
	...
	for _, e := range entries {
		res, err := stmt.Exec(e.Package, e.Version, e.Ecosystem, e.Feed, e.Severity, e.Reason, e.Detected.UTC().Format("2006-01-02 15:04:05"))
		if err != nil {
...
func (s *Store) Search(pkg, ecosystem string) ([]ThreatEntry, error) {
	query := `SELECT id, package, version, ecosystem, feed, severity, reason, detected, created
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.Package, &t.Version, &t.Ecosystem, &t.Feed, &t.Severity, &t.Reason, &detectedStr, &createdStr); err != nil {
